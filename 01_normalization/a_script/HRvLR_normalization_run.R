#!/usr/bin/env Rscript
# ============================================================================
# HRvLR Proteomics — Normalization Pipeline (Standalone Runner)
# High Responders vs. Low Responders to Resistance Training —
# Skeletal Muscle DIA-MS Proteomics
#
# 1:1 reproduction of HRvLR_normalization.qmd as a clean R script.
# Run from: 01_normalization/a_script/
# Usage:    source("HRvLR_normalization_run.R")  OR  Rscript HRvLR_normalization_run.R
#
...
cat("=== HRvLR Normalization Pipeline ===\n\n")
...
# Color palette
pal_group <- c(HR = "#B2182B", LR = "#2166AC")
pal_group_time <- c(
  HR_T1 = "#F4A582", HR_T2 = "#D6604D", HR_T3 = "#B2182B",
  LR_T1 = "#92C5DE", LR_T2 = "#4393C3", LR_T3 = "#2166AC"
)
...
# Raw intensity data
raw <- read_excel(file.path(input_dir, "HRvLR_raw.xlsx"))
...
# Metadata
metadata <- read.csv(file.path(input_dir, "HRvLR_meta.csv"),
...
# Paired missingness deltas for all subjects (fully paired design)
subjects <- unique(dal$metadata$Subject_ID)

delta_rows <- list()
for (subj in subjects) {
  rows <- dal$metadata %>% filter(Subject_ID == subj) %>% arrange(Timepoint)
  subj_ids <- rows$Col_ID
  subj_miss <- pct_missing[subj_ids]

  for (k in seq_along(subj_ids)) {
    delta_val <- if (k == 1) 0 else subj_miss[k] - subj_miss[1]
...
# IQR-based flags on delta missingness (exclude T1 baselines where delta = 0)
delta_vals <- delta_missing$delta_missing[delta_missing$Timepoint != "T1"]
...
p1 <- ggplot(outlier_diag,
  ...
  labs(x = "% Missing", y = "Delta Missing (vs T1 baseline)",
...
cat("\n=== HRvLR Normalization Pipeline Complete ===\n")
