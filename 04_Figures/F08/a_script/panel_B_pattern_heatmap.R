# F08 Panel B: Acute Response Pattern Heatmap
# Layout: [legends] [sig|heatmap|quad] [sankey] [stacked bars in striped bg]
# 3 groups: Concordant Up, Concordant Down, Discordant
# Filters DEPs significant in any Acute contrast (Acute_HR, Acute_LR, Acute_Interaction)
...
sig_df <- dep_df %>%
  filter(!is.na(logFC_Acute_HR), !is.na(logFC_Acute_LR)) %>%
  filter(pi_score_Acute_HR < 0.05 | pi_score_Acute_LR < 0.05 |
         pi_score_Acute_Interaction < 0.05) %>%
  mutate(
    quadrant = case_when(
      logFC_Acute_HR > 0 & logFC_Acute_LR > 0 ~ "Concordant Up",
      logFC_Acute_HR < 0 & logFC_Acute_LR < 0 ~ "Concordant Down",
      TRUE ~ "Discordant"
    ),
    sig_cat = case_when(
      pi_score_Acute_HR < 0.05 & pi_score_Acute_LR < 0.05 ~ "Both",
      pi_score_Acute_HR < 0.05  ~ "Ac.(HR)",
      pi_score_Acute_LR < 0.05  ~ "Ac.(LR)",
      pi_score_Acute_Interaction < 0.05 ~ "Inter.",
      TRUE ~ "NS"
    )
  )
...
SIG_COLORS <- c("Both" = "#2E7D32", "Ac.(HR)" = "#67A9CF",
                "Ac.(LR)" = "#EF8A62", "Inter." = "#FF8F00", "NS" = "grey70")
...
sig_df <- sig_df %>%
  left_join(mem_result$gene_map %>%
              transmute(gene, pathway), by = "gene") %>%
  mutate(pathway = ifelse(is.na(pathway), "Other", as.character(pathway)))

sig_df <- sig_df %>%
  mutate(quadrant = factor(quadrant, levels = QUAD_ORDER)) %>%
  arrange(quadrant, pathway, desc(logFC_Acute_HR))
...
if (n_pw == 0) {
  message("No pathways with >= 2 proteins; skipping pattern heatmap")
...
dom_quad <- sig_df %>%
  ...
  summarise(n = n(),
            lfc_sum = sum(abs(logFC_Acute_HR) + abs(logFC_Acute_LR)),
            .groups = "drop") %>%
  group_by(pathway) %>%
...
# Sigmoid ribbon helper (inline)
make_sigmoid_ribbon <- function(x_left, x_right, y_top_l, y_bot_l,
                                 y_top_r, y_bot_r, n_pts = 60,
                                 ribbon_id = "r") {
  t_seq <- seq(0, 1, length.out = n_pts)
  s <- 1 / (1 + exp(-12 * (t_seq - 0.5)))
  x_seq <- x_left + t_seq * (x_right - x_left)
  top <- y_top_l + s * (y_top_r - y_top_l)
  bot <- y_bot_l + s * (y_bot_r - y_bot_l)
  tibble(x = c(x_seq, rev(x_seq)),
         y = c(top, rev(bot)),
         ribbon_id = ribbon_id)
}
...
# 7. HEATMAP TILE DATA
# =============================================================================
fc_max <- max(abs(c(sig_df$logFC_Acute_HR, sig_df$logFC_Acute_LR)), na.rm = TRUE)
...
heat_tiles <- bind_rows(
  sig_df %>% transmute(x = X_COL1, y, w = TILE_W, h = ROW_H,
                        fill = lfc_to_color(logFC_Acute_HR, fc_max)),
  sig_df %>% transmute(x = X_COL2, y, w = TILE_W, h = ROW_H,
                        fill = lfc_to_color(logFC_Acute_LR, fc_max))
)
sig_tiles <- sig_df %>%
...
sig_leg_cats <- c("Both", "Ac.(HR)", "Ac.(LR)", "Inter.")
...
p <- ggplot() +
  ...
  coord_cartesian(xlim = c(-0.8, X_BAR_MAX + 1.0),
                  ...
                  expand = FALSE) +
  labs(title = "Acute Response Patterns",
...
sig_df %>%
  transmute(gene, quadrant = as.character(quadrant), sig_cat, pathway,
            logFC_Acute_HR = round(logFC_Acute_HR, 4),
            logFC_Acute_LR = round(logFC_Acute_LR, 4)) %>%
...
message("F08 Panel B (pattern heatmap) done")
