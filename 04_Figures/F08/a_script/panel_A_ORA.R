...
ora_q1 <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Concordant Up"],
                       "Concordant Up")
ora_q2 <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Discordant (HR\u2193 LR\u2191)"],
                       "Discordant (HR Down LR Up)")
ora_q3 <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Concordant Down"],
                       "Concordant Down")
ora_q4 <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Discordant (HR\u2191 LR\u2193)"],
...
label_df <- sig_df %>%
  ...
  slice_head(n = 5) %>%
  ungroup() %>%
  mutate(label_fill     = SIG_LABEL_FILL_F08[as.character(sig_class)],
...
composite <- p_ul + p_scatter + p_ur + p_ll + p_lr +
  ...
  plot_annotation(
    ...
    subtitle = sprintf("Threshold-free ORA | N = %d | %d DEPs (Pi < 0.05) | %d enriched (FDR < 0.05) | r = %.2f",
