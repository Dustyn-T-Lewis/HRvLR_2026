# F08 Panel D: fGSEA NES Scatter — Acute Enrichment (Acute_HR vs Acute_LR)
# GO:BP + Hallmark a priori collection
...
# -- Run fGSEA on both Acute contrasts ----------------------------------------
...
# Save full fGSEA cache
dir.create(file.path(DAT, "shared"), recursive = TRUE, showWarnings = FALSE)
fgsea_all %>%
  select(-leadingEdge) %>%
  write_csv(file.path(DAT, "shared", "fgsea_tstat_acute.csv"))

# Filter to a priori: Hallmark + GO:BP
...
fgsea_wide <- fgsea_all %>%
  filter(database %in% use_dbs,
         contrast %in% c("Acute_HR", "Acute_LR")) %>%
...
fgsea_wide <- fgsea_wide %>%
  mutate(
    ...
    significance = case_when(
      ...
      sig_LR          ~ "Sig LR only",
      TRUE            ~ "NS"
    ) %>% factor(levels = names(SIG_COLORS_F08)),
...
message(sprintf("  %d total pathways (%s) | %d significant",
                nrow(fgsea_wide), paste(use_dbs, collapse = " + "), nrow(fgsea_sig)))

# Spearman correlation
nes_cor_all <- cor.test(fgsea_wide$NES_Acute_HR, fgsea_wide$NES_Acute_LR,
                         method = "spearman")
nes_ci_all  <- fisher_z_ci(nes_cor_all$estimate, nrow(fgsea_wide))
nes_cor_sig <- if (nrow(fgsea_sig) >= 3) {
  cor.test(fgsea_sig$NES_Acute_HR, fgsea_sig$NES_Acute_LR, method = "spearman")
} else NULL

nes_lim <- max(abs(c(fgsea_wide$NES_Acute_HR, fgsea_wide$NES_Acute_LR))) * 1.15

# Quadrant counts (sig terms only)
n_conc_tr  <- sum(fgsea_sig$NES_Acute_HR > 0 & fgsea_sig$NES_Acute_LR > 0)
n_conc_bl  <- sum(fgsea_sig$NES_Acute_HR < 0 & fgsea_sig$NES_Acute_LR < 0)
n_disc_q2  <- sum(fgsea_sig$NES_Acute_HR < 0 & fgsea_sig$NES_Acute_LR > 0)
n_disc_q4  <- sum(fgsea_sig$NES_Acute_HR > 0 & fgsea_sig$NES_Acute_LR < 0)
...
# Labels for sig terms
label_pw <- fgsea_sig %>%
  mutate(
    label_fill     = SIG_LABEL_FILL_F08[as.character(significance)],
    label_text_col = SIG_LABEL_TEXT_F08[as.character(significance)]
  )
...
subtitle_str <- sprintf(
  "%s (a priori) | %d pathways (%d sig.) | fGSEA on limma t-statistics\n\u03c1(all) = %.2f [%.2f, %.2f], %s%s | %.0f%% concordant",
  paste(use_dbs, collapse = " + "), nrow(fgsea_wide), n_total_sig,
  nes_cor_all$estimate, nes_ci_all[1], nes_ci_all[2],
...
pD <- ggplot(mapping = aes(x = NES_Acute_HR, y = NES_Acute_LR)) +
  ...
  labs(title = "Pathway-Level Concordance: Acute (fGSEA)",
...
fgsea_wide %>%
  transmute(
    ...
    NES_Acute_HR   = round(NES_Acute_HR, 3),
    NES_Acute_LR   = round(NES_Acute_LR, 3),
    padj_Acute_HR  = signif(padj_Acute_HR, 4),
    padj_Acute_LR  = signif(padj_Acute_LR, 4),
    significance   = as.character(significance),
    set_size
  ) %>%
  arrange(significance, desc(abs(NES_Acute_HR) + abs(NES_Acute_LR))) %>%
  write_csv(file.path(DAT, "panel_D", "nes_scatter.csv"))

message("F08 Panel D done")
