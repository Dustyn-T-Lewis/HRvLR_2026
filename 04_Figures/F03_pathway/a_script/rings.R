...
ring_plot <- function(fg, dep, pw, ct, palette, tag = NULL, title = ct, bg = NULL) {
  prep <- ring_enrich(fg, ct, pw)
  n_sig <- nrow(ring_significant(fg, ct))
  n_drawn <- nrow(prep$enrich)
  subtitle <- if (n_sig == 0) {
    sprintf("0 pathways at BH < 0.05; %d nearest shown (n.s.)", n_drawn)
  } else {
    sprintf("%d pathways at BH < 0.05; top %d shown", n_sig, n_drawn)
  }
  ...
  # Minimal per-panel wash groups the composite by arm (or by contrast family) without
  # a heavy facet strip.
  if (!is.null(bg)) {
    p <- p + ggplot2::theme(plot.background = ggplot2::element_rect(fill = bg, color = NA))
  }
...
build_ring_grid <- function(fg, dep, pw, specs, ncol, byrow = TRUE, guide_area = FALSE, legend = NULL) {
  panels <- lapply(specs, function(s) {
    ring_plot(fg, dep, pw, s$contrast, s$palette, s$tag, title = s$title %||% s$contrast, bg = s$bg)
  })
  plots <- lapply(panels, `[[`, "plot")
  # A 5-ring grid leaves the last cell of a 3x2 empty; hand it to a guide_area so the
  # legends sit there instead of stealing height from the bottom row. A full grid puts
  # the single legend on the right.
  legend_pos <- legend %||% (if (guide_area) "right" else "bottom")
  if (guide_area) plots <- c(plots, list(patchwork::guide_area()))
  grid <- patchwork::wrap_plots(
    ...
    plot.margin = ggplot2::margin(2, 8, 2, 8, "mm")
  )
