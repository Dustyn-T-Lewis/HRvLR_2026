...
subtitle: "The five responder contrasts as volcano-in-ring plots; the four within-group responses as the positive control"
author: "A_HRvLR_2026"
date: today
...
trained, and acute states. Nine contrasts are estimable: the five that test responder
status carry the main figure, and the four within-group responses sit in the supplement as
the positive control. Each ring is keyed to a family palette so a reader never mistakes one
kind of comparison for another.
...
## The main figure, and what it deliberately leaves out

Nine contrasts are estimable. Only **five** of them test the question this study asks —
*does a high responder's proteome differ from a low responder's?* Those five are the main
figure. The other four describe what training does *inside* each arm, which is a different
question, and they sit in the supplement.

The main figure holds the five responder contrasts:

- **Differential** (green up, purple down) — `Baseline_HRvLR`, `Trained_HRvLR`,
  `Acute_HRvLR` (A–C), read as the untrained, trained, and acute states.
- **Interaction** (orange up, purple down) — `Training_Interaction`, `Acute_Interaction`
  (D–E), where the two groups' *responses* diverge.

![F03. The five contrasts that test responder status: HR vs LR at baseline, trained and acute (A–C), and the two differential-response interactions (D–E).](../b_reports/F03_pathway.png)

The supplement holds the four within-group responses — what moves inside HR and inside LR
over training (T2 − T1) and over the acute bout (T3 − T2), in red-up / blue-down.

**These are the positive control, and they are why the main figure is worth reading.** A
null between responders means nothing if nothing moved at all; it would just mean the
assay, or the study, was too weak to see anything. Because the within-group responses *are*
strongly enriched, the absence of a between-responder difference is **informative** rather
than merely underpowered. That is the whole argument, and it is why these four rings are
kept and shown rather than dropped.

![Supplement: the four within-group responses (positive control). Training and the acute bout do move the proteome inside each arm — which is what makes the null in the main figure meaningful rather than merely underpowered.](../b_reports/supp/within_group_responses.png)
...
- `b_reports/F03_pathway.{png,pdf}` — the five responder contrasts (three HRvLR + two interactions).
- `b_reports/supp/within_group_responses.{png,pdf}` — the four within-group responses (positive control).
...
F04_association/WGCNA and F04_association_HLM NES scatters read instead of rerunning GSEA. The
enrichment is paid for once per F03_pathway run, and the three figures cannot drift apart. **Run
F03_pathway before F04_association/WGCNA and F04_association_HLM.**
...
|---|---|---|---|
| **Main** (A–C) | HR and LR proteomes do not differ at baseline, trained, or acute | Ring arcs: fgsea preranked GSEA on moderated-*t* ranks [@korotkevich2021] against Hallmark, KEGG, Reactome, GO:BP/CC/MF and GO Slim (MSigDB [@liberzon2015]), BH-adjusted **within contrast**. Volcano points: π-gated | Sound, with one caveat that must be read: **the arcs and the points are different statistics.** Arcs carry a genuine fgsea BH *q*. Points carry π, which is **not** an FDR. |
...
| **Supp** within-group | Training and the acute bout do move the proteome | Same fgsea machinery, applied within each arm | This is the **positive control**, and it is why the main figure's null is interpretable. If nothing moved anywhere, the HRvLR null would just mean "underpowered." Because the within-group responses *are* enriched, the absence of a between-responder difference is informative. |
...
**not** a subset of *p* < 0.05: **14.5 % of π-calls have raw *p* ≥ 0.05.** It is a
