...
meta <- read_csv(here("00_input", "HRvLR_meta.csv"), show_col_types = FALSE) %>%
  filter(Timepoint %in% c("T1", "T2"), !is.na(fCSA_Mixed_Pre)) %>%
  mutate(
...
pC_left <- ggplot(meta, aes(x = Group_Time, y = value, fill = Group_Time)) +
  geom_bar(
  ...
  labs(
    title = "fCSA Mixed", subtitle = anova_sub,
    y = expression("fCSA (" * mu * m^2 * ")"), x = NULL, tag = "C"
  ) +
  FIG_THEME +
  theme(
...
pC_right <- ggplot(pheno_wide, aes(x = Group, y = delta, fill = Group)) +
  geom_bar(
...
save_panel(pC, file.path(RPT, "panels", "panel_c_fcsa_mixed"), PW, PH)
