...
meta <- read_csv(here("00_input", "HRvLR_meta.csv"), show_col_types = FALSE) %>%
  filter(Timepoint %in% c("T1", "T2"), !is.na(fCSA_Type_I_Pre)) %>%
  mutate(
...
pD_left <- ggplot(meta, aes(x = Group_Time, y = value, fill = Group_Time)) +
  geom_bar(
  ...
  labs(
    title = "fCSA Type I", subtitle = anova_sub,
    y = expression("fCSA (" * mu * m^2 * ")"), x = NULL, tag = "D"
  ) +
  FIG_THEME +
  theme(
...
pD_right <- ggplot(pheno_wide, aes(x = Group, y = delta, fill = Group)) +
  geom_bar(
...
save_panel(pD, file.path(RPT, "panels", "panel_d_fcsa_type1"), PW, PH)
