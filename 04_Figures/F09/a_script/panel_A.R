...
x <- 0
for (i in seq_len(n_lmm)) {
  x <- x + 1
  xpos[i] <- x
}
for (i in seq_len(n_bl_hr)) {
  x <- x + (if (i == 1) 1 + gap else 1)
  xpos[n_lmm + i] <- x
}
for (i in seq_len(n_bl_lr)) {
  x <- x + (if (i == 1) 1 + gap else 1)
  xpos[n_lmm + n_bl_hr + i] <- x
}
for (i in seq_len(n_ch_hr)) {
  x <- x + (if (i == 1) 1 + gap else 1)
  xpos[n_lmm + n_bl_hr + n_bl_lr + i] <- x
}
for (i in seq_len(n_ch_lr)) {
  x <- x + (if (i == 1) 1 + gap else 1)
  xpos[n_lmm + n_bl_hr + n_bl_lr + n_ch_hr + i] <- x
}
...
heat_df <- expand.grid(
  module = rownames(cor_mat), trait = colnames(cor_mat),
...
p_counts <- ggplot(mod_counts, aes(x = n_proteins, y = module)) +
  ...
  FIG_THEME +
  theme(
    axis.text.y = element_text(size = txt_cell * 1.3, face = "bold"),
...
p_heat <- ggplot(heat_df, aes(x = xpos, y = module, fill = cor)) +
  ...
  annotate("rect",
    xmin = sec_lmm_start - 0.5, xmax = sec_lmm_end + 0.5,
    ymin = -Inf, ymax = Inf, fill = "grey70", alpha = 0.06
  ) +
  annotate("rect",
    xmin = sec_blhr_start - 0.5, xmax = sec_blhr_end + 0.5,
    ymin = -Inf, ymax = Inf, fill = GROUP_COLORS["HR"], alpha = 0.06
  ) +
  annotate("rect",
    xmin = sec_bllr_start - 0.5, xmax = sec_bllr_end + 0.5,
    ymin = -Inf, ymax = Inf, fill = GROUP_COLORS["LR"], alpha = 0.06
  ) +
  annotate("rect",
    xmin = sec_chhr_start - 0.5, xmax = sec_chhr_end + 0.5,
    ymin = -Inf, ymax = Inf, fill = GROUP_COLORS["HR"], alpha = 0.06
  ) +
  annotate("rect",
    xmin = sec_chlr_start - 0.5, xmax = sec_chlr_end + 0.5,
    ymin = -Inf, ymax = Inf, fill = GROUP_COLORS["LR"], alpha = 0.06
  ) +
  geom_tile(color = "black", linewidth = 0.3) +
  # Red/blue border for highest/lowest r per row
  geom_tile(
    data = heat_df %>% group_by(module) %>% filter(cor == max(cor)) %>% slice(1) %>% ungroup(),
    color = "#B2182B", linewidth = 1.3, fill = NA
  ) +
  geom_tile(
    data = heat_df %>% group_by(module) %>% filter(cor == min(cor)) %>% slice(1) %>% ungroup(),
    color = "#2166AC", linewidth = 1.3, fill = NA
  ) +
  # Solid thick black border for BH FDR < 0.05
  geom_tile(
    data = heat_df %>% filter(pval < 0.05),
    color = "black", linewidth = 1.0, fill = NA
  ) +
  # Dot for nominal p < 0.05 (but FDR >= 0.05)
  geom_point(
    data = heat_df %>% filter(pval_raw < 0.05 & pval >= 0.05),
...
pA <- pA +
  plot_annotation(
    ...
    subtitle = sprintf(
      "%d modules (Pearson, power %d) | LMM: lmer + KR df, BH-corrected | Stratified: bicor, BH per-trait",
      length(mod_order), sft_csv$selected_power[1]
    ),
    caption = paste0(
      "Values: r or r-equiv. | BH per-column (LMM: ", nrow(lmm_audit),
...
ggsave(file.path(RPT, "panel_a_module_trait.pdf"), pA,
  width = PA_W, height = PA_H, units = "mm",
  device = pdf_device, limitsize = FALSE, bg = "white"
)
ggsave(file.path(RPT, "panel_a_module_trait.png"), pA,
  width = PA_W, height = PA_H, units = "mm",
