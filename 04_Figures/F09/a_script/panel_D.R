...
assign_groups_ora <- function(gene_names, max_groups = 4, min_group_n = 3) {
  ...
  gene_map <- data.frame(
    gene = character(), pathway = character(),
...
build_network_hull <- function(mod) {
  ...
  adj_mod <- adjacency(datExpr[, mod_prots],
    power = NET_POWER, type = "signed hybrid",
    corFnc = "cor"
  )
  ...
  disp_label <- if (mod %in% names(display_label_vec)) {
    display_label_vec[[mod]]
  } else {
    str_to_title(mod)
  }
...
for (mod in KEY_MODULES) {
  res <- results[[mod]]
  fname <- sprintf("panel_c_hub_%s", mod)
...
for (mod in KEY_MODULES) {
  ...
  for (j in seq_len(nrow(nd))) {
    ...
    gs_ct <- if (length(valid) >= 4) {
      cor.test(prot_expr[valid], pheno_vec[valid],
        method = "pearson"
      )
    } else {
      NULL
