...
run_fry_set <- function(idx, nm) {
  if (length(idx) < 3) {
    return(tibble(set = nm, n = length(idx), direction = NA, PValue = NA_real_))
  }
  res <- fry(mat_imp,
    index = idx, design = design, contrast = cm[, "Acute_LR"],
    block = block_id, correlation = cor_imp
  )
  tibble(
    set = nm, n = length(idx), direction = res$Direction[1],
...
t_rank <- dep_df %>%
  ...
  mutate(
    rank = row_number(), in_up = uniprot_id %in% sets_pi$up_ids,
...
make_bc <- function(t_df, in_col, es_col, fr, title, col) {
  ...
  p_es <- ggplot(t_df, aes(x = rank, y = .data[[es_col]])) +
    geom_area(fill = scales::alpha(lc, 0.15)) +
    geom_line(color = lc, linewidth = 0.6) +
...
p1 <- make_bc(
  t_rank, "in_up", "es_up", fry_up,
  sprintf("Ac.(HR)-Up DEPs (n=%d) -> Ac.(LR) ranked t", length(sets_pi$up)), HR_COL
)
p2 <- make_bc(
  t_rank, "in_down", "es_down", fry_dn,
  sprintf(
    "Ac.(HR)-Down DEPs (n=%d) -> Ac.(LR) ranked t%s",
    length(sets_pi$down),
...
pC <- p1$es / p1$bc / p2$es / p2$bc / p_t +
  ...
  plot_annotation(
    title = "fry: Acute Concordance (HR-sig in LR)",
