...
# Generates: panel_D_dendrogram_SUPP.pdf/png

setwd(rprojroot::find_rstudio_root_file())
source("04_Figures/F09/a_script/style.R")

suppressPackageStartupMessages({
  library(readr); library(dplyr); library(tibble); library(WGCNA); library(png)
})
...
net           <- readRDS(file.path(WGCNA_DATA, "wgcna_network.rds"))
module_df     <- read_csv(file.path(WGCNA_DATA, "wgcna_module_assignments.csv"),
                          show_col_types = FALSE)
module_colors <- readRDS(file.path(PANEL_DATA, "module_colors.rds"))
...
ggsave(file.path(RPT_SUPP_PDF, "a01_dendrogram_SUPP.pdf"), pA,
       width = PA_W, height = PA_H, units = "mm",
       device = pdf_device, limitsize = FALSE)
ggsave(file.path(RPT_SUPP_PNG, "a01_dendrogram_SUPP.png"), pA,
       width = PA_W, height = PA_H, units = "mm",
