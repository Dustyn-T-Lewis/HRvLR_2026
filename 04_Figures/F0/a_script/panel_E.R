# F0 Panel E: fCSA Type II (Pre/Post x Group + Delta) ----
...
meta <- read_csv("00_input/HRvLR_meta.csv", show_col_types = FALSE) %>%
  filter(Timepoint %in% c("T1", "T2"), !is.na(fCSA_Type_II_Pre)) %>%
  mutate(
...
pE_left <- ggplot(meta, aes(x = Group_Time, y = value, fill = Group_Time)) +
  ...
  labs(title = "fCSA Type II", subtitle = anova_sub,
       y = expression("fCSA (" * mu * m^2 * ")"), x = NULL, tag = "E") +
  FIG_THEME +
...
pE_right <- ggplot(pheno_wide, aes(x = Group, y = delta, fill = Group)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.25)),
                     labels = scales::label_comma()) +
  labs(y = expression(Delta ~ "fCSA (" * mu * m^2 * ")"), x = NULL) +
  FIG_THEME + theme(legend.position = "none")

pE <- (pE_left | pE_right) + plot_layout(widths = c(0.65, 0.35))

# Save ----
save_panel(pE, file.path(RPT, "panel_E_fcsa_type2"), PW, PH)
