...
F05 is the acute arm of the concordance comparison. Where F04 asked how HR and LR
proteomes move over a training block (T1 → T2), F05 asks how they move in the hour
...
might reasonably differ. F05 tests that.

Divergence is read off the modelled interaction contrast, not off the eye. F05 and
F04 call the same driver, `render_concordance_figure()`, and differ only in
...
acute logFC vectors is **ρ = 0.15** (bootstrap 95% CI 0.11–0.20), below F04's 0.20.
...
HR-down direction is the closest thing to a signal anywhere in F04 or F05, and it is
...
anywhere in F04/F05 — and 10 of 57 pathways are flagged divergent, against 2 in F04.
...
The honest headline is unchanged from F04: **at n = 8 per group this figure does not
establish a responder-specific acute program.** It generates leads.

## Composite

![F05 composite. A quadrant concordance map with per-quadrant ORA, B protein-to-pathway assignment, C the fry rotation test, D pathway NES concordance, E threshold-free RRHO2.](../b_reports/F05_composite.png)
...
- `04_Figures/F03/c_data/F03_source_data.xlsx` (sheet `fgsea_all`) — the fgsea cache,
  computed once in F03 and reused so no permutation test reruns.

Writes, under `04_Figures/F05/`: panel and supplementary PNG + PDF, one `c_data/*.csv`
per panel, and the bundled `c_data/F05_source_data.xlsx`.
...
The gap is wider here than in F04 (0.15 → 0.31, versus 0.20 → 0.28), which is what you
...
**How to read it.** The cloud is rounder than F04's. Note the asymmetry: the two
...
F04, where the selected proteins scattered thinly across categories. The acute response
...
interaction GSEA — five times F04's count.
...
46 concordant-down); the discordant corners hold 3. This is far smaller than F04's 694,
...
27 concordant-up pathways. As in F04, a hot concordant corner is expected whenever two
...
`04_Figures/F03/a_script/ring_helpers.R`, a few lines from a genuine fgsea BH q-value. The
...
- F05 does not establish a responder-specific acute program. Stating that is the result.
