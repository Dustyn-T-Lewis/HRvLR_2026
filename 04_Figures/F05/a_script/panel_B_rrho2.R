# F05 Panel B: RRHO2 Threshold-Free Concordance Heatmaps
# Two side-by-side RRHO2 maps: Training (HR vs LR) + Acute (HR vs LR)
...
make_rrho2 <- function(dep, pair, half_w) {
  ...
  # Hotspot genes from RRHO2 package quadrant gene lists
  ...
  # Annotation positions (centered in each quadrant region)
...
message("F05 Panel B (RRHO2) done")
