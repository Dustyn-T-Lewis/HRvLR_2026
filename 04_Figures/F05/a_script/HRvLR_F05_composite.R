# Stitch the F05 composite from the rendered panels (A concordance map + flanking
...
save_panel(composite, file.path(RPT_DIR, "F05_composite"), 340, 320)
...
overview <- data.frame(
  sheet = names(sheets),
  description = c(
    "Panel A: per-protein HR/LR acute logFC, quadrant, divergence class",
...
saveWorkbook(wb, file.path(DAT_DIR, "F05_source_data.xlsx"), overwrite = TRUE)
cat("F05 composite + workbook written.\n")
