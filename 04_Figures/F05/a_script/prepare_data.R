# F05 Between-Group Concordance — Data Preparation
...
pairs <- list(
  Training = list(x = "Training_HR", y = "Training_LR",
                  label_x = "HR", label_y = "LR"),
  Acute    = list(x = "Acute_HR",    y = "Acute_LR",
                  label_x = "HR", label_y = "LR")
)
...
write_csv(conc_stats, file.path(DAT, "concordance_stats.csv"))

message("prepare_data.R done")
message(sprintf("  Training: r=%.3f, rho=%.3f, sign=%.1f%%",
                conc_stats$pearson_all[1], conc_stats$spearman_all[1],
                conc_stats$sign_concordance_pct[1]))
message(sprintf("  Acute:    r=%.3f, rho=%.3f, sign=%.1f%%",
