...
F04_association/WGCNA module eigengenes, under an elastic net and a sparse PLS regression.
...
The **F04_association/WGCNA module eigengenes were not built inside the cross-validation
...
metrics <- map_dfr(phases, function(ph) {
  read_csv(
    here(
      "04_Figures", "F05_prediction", "prediction_continuous", ph, "c_data",
      "cont_metrics.csv"
    ),
...
cells.** Each phase is a separate `01_run_<phase>.R`, and `p.adjust()` is applied within
...
(`d_mcsa`) predicted from the F04_association/WGCNA module eigengenes under the elastic net, in
...
built, even though its outcome label was never seen. The F04_association/WGCNA eigengenes are
