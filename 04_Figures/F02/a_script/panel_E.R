...
mask_file <- "02_Imputation/c_data/07_imputation_mask.csv"
has_mask  <- file.exists(mask_file)
if (has_mask) {
  mask_df  <- read_csv(mask_file, show_col_types = FALSE)
...
has_mnar  <- file.exists(mnar_file)
if (has_mnar) {
  mnar_df   <- read_csv(mnar_file, show_col_types = FALSE)
...
lfc_list <- lapply(subjects, function(s) {
  ...
  n_imputed_lfc <- 0; pct_imputed <- 0
  n_mnar_lfc <- 0; pct_mnar <- 0
...
subj_summary <- lfc_long |>
  group_by(subject, resp_group, n_imputed_lfc, pct_imputed,
           n_mnar_lfc, pct_mnar) |>
  summarise(
...
write.csv(subj_summary |>
            select(subject, resp_group, median_lfc, mad_lfc, sd_lfc,
