...
GROUP_COLS  <- c(HR_T1 = scales::alpha("#2166AC", 0.40), HR_T2 = "#2166AC",
                 LR_T1 = scales::alpha("#B2182B", 0.40), LR_T2 = "#B2182B")
...
bracket_comps <- list(c("HR_T1", "HR_T2"), c("LR_T1", "LR_T2"),
                      c("HR_T1", "LR_T1"), c("HR_T2", "LR_T2"))
...
# Delta median CV per group (T2 - T1)
delta_cv <- cv_ci |>
...
sub_txt <- sprintf(
  ...
  delta_cv$delta[delta_cv$resp_group == "HR"],
  delta_cv$delta[delta_cv$resp_group == "LR"]
)
