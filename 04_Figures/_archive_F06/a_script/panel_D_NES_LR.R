# F06 Panel D (LR): fGSEA NES Scatter — Chronic vs Acute
# Training_LR (x) vs Acute_LR (y) pathway concordance
...
GRP <- "LR"; CTR_X <- "Training_LR"; CTR_Y <- "Acute_LR"
PG_W <- 200
...
fgsea_hg <- fgsea_all %>% filter(database %in% use_dbs, contrast %in% c(CTR_X, CTR_Y))
...
pD <- ggplot(mapping = aes(x = .data[[paste0("NES_", CTR_X)]],
  ...
  annotate("label", x = Inf, y = Inf,
           label = sprintf("Concordant Up  n = %d",
                            sum(fgsea_sig[[paste0("NES_", CTR_X)]] > 0 & fgsea_sig[[paste0("NES_", CTR_Y)]] > 0)),
           hjust = 1.05, vjust = 1.2, size = txt_quad, fontface = "bold",
           color = "#D6604D", fill = alpha("white", 0.92), label.padding = unit(2.5, "pt")) +
  annotate("label", x = -Inf, y = -Inf,
           label = sprintf("Concordant Down  n = %d",
                            sum(fgsea_sig[[paste0("NES_", CTR_X)]] < 0 & fgsea_sig[[paste0("NES_", CTR_Y)]] < 0)),
