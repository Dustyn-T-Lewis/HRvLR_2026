# F06 Panel C (HR): fry Rotation Test — Training-Sig Tested in Acute
# Defines gene sets from Training_HR Pi<0.05, tests against Acute_HR contrast.
...
GRP <- "HR"; CTR_SIG <- "Training_HR"; CTR_TEST <- "Acute_HR"
...
cm <- makeContrasts(Acute_HR = HR_T3 - HR_T2, levels = design)
...
run_fry_set <- function(idx, set_name) {
  ...
  res <- fry(mat_imp, index = idx, design = design,
             contrast = cm[, "Acute_HR"], block = block_id, correlation = cor_imp)
...
make_barcode <- function(t_df, in_col, es_col, fry_row, title, color) {
  ...
  p_es <- ggplot(t_df, aes(x = rank, y = .data[[es_col]])) +
    ...
    annotate("text", x = n_all * 0.98, y = Inf,
             label = sprintf("fry %s, %s (n = %d)", fry_row$direction,
...
up_title <- sprintf("Tr.(%s)-Up DEPs (n = %d) -> %s ranked t",
                     GRP, length(sets_pi$up), CTR_SHORT[CTR_TEST])
dn_title <- sprintf("Tr.(%s)-Down DEPs (n = %d) -> %s ranked t%s",
...
pC <- p1$es / p1$bc / p2$es / p2$bc / p_t +
  ...
  plot_annotation(
    ...
    subtitle = sprintf("dupCor = %.3f | n = %d proteins | %s-sig sets tested in %s",
