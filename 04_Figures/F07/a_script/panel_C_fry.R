...
driving_up <- dep_df %>%
  filter(uniprot_id %in% sets_pi$up_ids, uniprot_id %in% imp_ids,
         t_Training_LR > 0) %>%
  transmute(gene, uniprot_id, set = "thr_up",
...
driving_dn <- dep_df %>%
  filter(uniprot_id %in% sets_pi$down_ids, uniprot_id %in% imp_ids,
         t_Training_LR < 0) %>%
  transmute(gene, uniprot_id, set = "thr_down",
            t_training_hr = t_Training_HR, t_training_lr = t_Training_LR,
            logFC_Training_HR, logFC_Training_LR, pi_score_Training_HR)
...
up_title <- sprintf("Tr.(HR)-Up DEPs (Pi < 0.05, n = %d) \u2192 Tr.(LR) ranked t",
                     length(sets_pi$up))
dn_title <- sprintf("Tr.(HR)-Down DEPs (Pi < 0.05, n = %d) \u2192 Tr.(LR) ranked t%s",
                     length(sets_pi$down),
...
p_t <- ggplot(t_rank, aes(x = rank, y = t_Training_LR)) +
  geom_area(fill = scales::alpha(CONTRAST_COLORS["Training_LR"], 0.20),
            color = CONTRAST_COLORS["Training_LR"], linewidth = 0.3) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.3) +
  labs(x = sprintf("Protein rank by t(Training LR)  [n = %d]", n_all),
       y = "t-stat") +
  scale_x_continuous(limits = c(1, n_all), expand = c(0.005, 0)) +
  FIG_THEME +
