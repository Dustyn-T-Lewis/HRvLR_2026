# F07 Panel C -- fry Rotation Test: Training Concordance Barcode
# Tests whether Training_HR-significant protein sets respond concordantly
# in LR subjects, using limma's fry gene-set rotation test.
#
...
#
# No circularity: Training_HR (HR_T2 - HR_T1) and Training_LR (LR_T2 - LR_T1)
# share no contrast terms.
#
...
dep_df <- read_csv("03_DEP/a_non_imputed/c_data/03_combined_results.csv",
...
# Block by Subject_ID for repeated measures
block_id <- meta$Subject_ID
...
# Contrast: Training_LR = LR_T2 - LR_T1 (test set for HR-defined gene sets)
cm <- makeContrasts(
...
sig_pi <- dep_df %>%
  filter(pi_score_Training_HR < 0.05, uniprot_id %in% imp_ids)

sets_pi <- list(
  up = match(sig_pi$uniprot_id[sig_pi$logFC_Training_HR > 0], imp_ids),
  down = match(sig_pi$uniprot_id[sig_pi$logFC_Training_HR < 0], imp_ids),
  up_ids = sig_pi$uniprot_id[sig_pi$logFC_Training_HR > 0],
  down_ids = sig_pi$uniprot_id[sig_pi$logFC_Training_HR < 0]
)
...
run_fry_set <- function(idx, set_name) {
  ...
  res <- fry(mat_imp,
    ...
    contrast = cm[, "Training_LR"], block = block_id,
    correlation = cor_imp
  )
  tibble(
...
fry_up <- run_fry_set(sets_pi$up, "thr_up") %>%
  mutate(expected = "Up", consistent = direction == expected)
fry_dn <- run_fry_set(sets_pi$down, "thr_down") %>%
...
driving_up <- dep_df %>%
  ...
  transmute(gene, uniprot_id,
    set = "thr_up",
    t_training_hr = t_Training_HR, t_training_lr = t_Training_LR,
    logFC_Training_HR, logFC_Training_LR, pi_score_Training_HR
  )
...
t_rank$es_up <- running_es(t_rank$t_Training_LR, t_rank$in_up)
t_rank$es_down <- running_es(t_rank$t_Training_LR, t_rank$in_down)
...
up_title <- sprintf(
  "Tr.(HR)-Up DEPs (Pi < 0.05, n = %d) \u2192 Tr.(LR) ranked t",
  length(sets_pi$up)
)
dn_title <- sprintf(
  "Tr.(HR)-Down DEPs (Pi < 0.05, n = %d) \u2192 Tr.(LR) ranked t%s",
  length(sets_pi$down),
  if (fry_dn$PValue > 0.05) "  (n.s.)" else ""
)

# Use HR color for the fry barcode curves
FIG_COLOR <- unname(CONTRAST_COLORS["Training_HR"])
...
p_t <- ggplot(t_rank, aes(x = rank, y = t_Training_LR)) +
  geom_area(
    fill = scales::alpha(CONTRAST_COLORS["Training_LR"], 0.20),
    color = CONTRAST_COLORS["Training_LR"], linewidth = 0.3
  ) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.3) +
  labs(
    x = sprintf("Protein rank by t(Training LR)  [n = %d]", n_all),
    y = "t-stat"
  ) +
  scale_x_continuous(limits = c(1, n_all), expand = c(0.005, 0)) +
  FIG_THEME +
...
pC_fry <- p1$es + p1$bc + p2$es + p2$bc + p_t +
  ...
  plot_annotation(
    ...
    subtitle = sprintf(
      "Rotation-based set test (exact GSEA analogue, accounts for inter-gene correlation) | dupCor = %.3f | n = %d proteins",
      cor_imp, n_all
    ),
    theme = theme(
...
if (!is.null(p_ora_up) || !is.null(p_ora_dn)) {
  ...
  ggsave(file.path(RPT, "panel_C_fry_ora_SUPP.pdf"), p_ora,
    width = 160, height = 100, units = "mm", device = pdf_device
  )
  ggsave(file.path(RPT, "panel_C_fry_ora_SUPP.png"), p_ora,
