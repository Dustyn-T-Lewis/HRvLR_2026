...
F04 is the training arm of that comparison. It maps the HR training response
...
F04 is the training figure (T1 → T2). Its acute sibling, F05, runs the same code
...
![F04 composite. A quadrant concordance map with per-quadrant ORA, B protein-to-pathway assignment of the selected proteins, C the fry rotation test, D pathway NES concordance, E threshold-free RRHO2.](../b_reports/F04_composite.png)
...
Writes, under `04_Figures/F04/`: `b_reports/panels/` and `b_reports/supp/` (PNG +
PDF), one `c_data/*.csv` per panel, and the bundled
`c_data/F04_source_data.xlsx`.
...
**How to read it.** This is the most *encouraging* panel in F04, and it needs the
...
named `padj` in `04_Figures/F03/a_script/ring_helpers.R`, a few lines from a
...
- F04 does not establish a responder-specific training program. Stating that is the
  result.
