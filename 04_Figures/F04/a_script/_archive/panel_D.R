# panel_D.R — Acute_LR Volcano + Ring
# Contrast: Acute_LR
...
p <- make_volcano_ring(
  ...
  contrast_title     = "Acute Response \u2014 LR",
  contrast_subtitle  = "LR: T3 \u2212 T1 (1h acute bout)",
...
save_panel(p, file.path(RPT, "panel_D_Acute_LR_MAIN"), PW, PH)
...
if (!is.null(ring_out) && nrow(ring_out) > 0)
  write_csv(ring_out |> dplyr::select(-gene_list),
            file.path(DAT, "panel_D", "ring_terms.csv"))
...
dep_df |>
  ...
  write_csv(file.path(DAT, "panel_D", paste0("volcano_Acute_LR.csv")))

message("Panel D done: Acute_LR")
