# panel_F.R — Training_Interaction Volcano + Ring
# Contrast: Training_Interaction
...
here::i_am(".here")
source(here::here("04_Figures/F03/a_script/style.R"))
source(here::here("04_Figures/F03/a_script/volcano_ring.R"))
...
p <- make_volcano_ring(
  ...
  contrast_title     = "Training Interaction (HR \u2212 LR)",
  contrast_subtitle  = "Training: HR \u2212 LR response",
...
save_panel(p, file.path(RPT, "panel_F_Training_Interaction_MAIN"), PW, PH)
...
if (!is.null(ring_out) && nrow(ring_out) > 0)
  write_csv(ring_out |> dplyr::select(-gene_list),
            file.path(DAT, "panel_F", "ring_terms.csv"))
...
dep_df |>
  ...
  write_csv(file.path(DAT, "panel_F", paste0("volcano_Training_Interaction.csv")))

message("Panel F done: Training_Interaction")
