# panel_E.R — Baseline_HRvLR Volcano + Ring
# Contrast: Baseline_HRvLR
...
p <- make_volcano_ring(
  ...
  contrast_title     = "Baseline: HR vs LR",
  contrast_subtitle  = "HR \u2212 LR at Baseline (T1)",
...
save_panel(p, file.path(RPT, "panel_E_Baseline_HRvLR_MAIN"), PW, PH)
...
if (!is.null(ring_out) && nrow(ring_out) > 0)
  write_csv(ring_out |> dplyr::select(-gene_list),
            file.path(DAT, "panel_E", "ring_terms.csv"))
...
dep_df |>
  ...
  write_csv(file.path(DAT, "panel_E", paste0("volcano_Baseline_HRvLR.csv")))

message("Panel E done: Baseline_HRvLR")
