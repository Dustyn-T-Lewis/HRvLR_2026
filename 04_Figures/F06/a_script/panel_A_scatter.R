# F06 Panel A: Within-Group Training-vs-Acute Concordance (HR + LR)
# Two side-by-side scatters: Training vs Acute within HR, Training vs Acute within LR
...
pairs <- list(
  HR = list(
    lfc_x = "logFC_Training_HR", lfc_y = "logFC_Acute_HR",
    pi_x  = "pi_score_Training_HR", pi_y = "pi_score_Acute_HR",
    t_x   = "t_Training_HR",  t_y   = "t_Acute_HR",
    label_x = "Training", label_y = "Acute",
    title = "High Responders"
  ),
  LR = list(
    lfc_x = "logFC_Training_LR", lfc_y = "logFC_Acute_LR",
    pi_x  = "pi_score_Training_LR", pi_y = "pi_score_Acute_LR",
...
half_w <- PA_W / 2
p_hr <- make_scatter(dep_df, pairs$HR, half_w)
p_lr <- make_scatter(dep_df, pairs$LR, half_w)

pA <- (p_hr | p_lr) +
  plot_annotation(
    title = "Protein-Level Concordance: Training vs Acute",
...
message("F06 Panel A (scatter) done")
