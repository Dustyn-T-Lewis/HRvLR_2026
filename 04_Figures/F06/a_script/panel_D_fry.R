# F06 Panel C: fry Rotation Test — Within-Group Acute Changes Barcode
# Tests whether training Pi-significant proteins align with acute response
# ranks within the same responder group.
#
# HR pair: Training_HR Pi-sig sets tested against Acute_HR t-stat ranks
# LR pair: Training_LR Pi-sig sets tested against Acute_LR t-stat ranks
#
# Design: training and acute contrasts are distinct within-group responses.
...
pair_defs <- list(
  HR = list(
    ...
    tgt_t   = "t_Acute_HR",
    src_label = "Training HR", tgt_label = "Acute HR",
    tgt_t2_pat = "HR.*T3", tgt_t1_pat = "HR.*T2"
  ),
  LR = list(
    src_lfc = "logFC_Training_LR", src_pi = "pi_score_Training_LR",
    tgt_t   = "t_Acute_LR",
    src_label = "Training LR", tgt_label = "Acute LR",
...
for (pair_name in names(pair_defs)) {
  ...
  sig_df <- dep_df %>%
    filter(!is.na(.data[[pd$src_pi]]) & .data[[pd$src_pi]] < 0.05,
  ...
  message(sprintf("  %s: %d up, %d down Pi-sig proteins", pair_name,
  ...
  for (dir in c("up", "down")) {
    ...
    if (length(idx) < 3) {
      fry_list[[dir]] <- tibble(pair = pair_name, set = dir, n = length(idx),
  ...
  fry_pair <- bind_rows(fry_list) %>%
    mutate(expected = ifelse(set == "up", "Up", "Down"),
  ...
  p_es_up <- ggplot(t_rank, aes(x = rank, y = es_up)) +
    ...
    labs(y = "ES", title = sprintf("%s-Up DEPs -> %s t-stats",
  ...
  p_t <- ggplot(t_rank, aes(x = rank, y = .data[[pd$tgt_t]])) +
    ...
    labs(x = sprintf("Rank by t(%s)  [%d proteins]", pd$tgt_label, n_all),
         y = "t-stat") +
    scale_x_continuous(limits = c(1, n_all), expand = c(0.005, 0)) +
    FIG_THEME +
...
if (length(barcode_plots) == 2) {
  pD <- (barcode_plots$HR | barcode_plots$LR) +
    plot_annotation(
      title = "fry Rotation Test: Training DEP sets against acute ranked t-statistics",
      subtitle = sprintf("missForest-imputed | dupCor=%.3f | Pi<0.05 gene sets",
                          cor_val),
      tag_levels = list(c("C", "")),
      theme = theme(
...
save_panel(pD, file.path(RPT, "panel_C_fry_MAIN"), PD_W, PD_H)

message("F06 Panel C (fry barcode) done")
