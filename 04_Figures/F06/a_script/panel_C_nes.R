# F06 Panel D: NES Scatter (GO Slim + Hallmark, a priori)
# Two side-by-side: Training_HR NES vs Acute_HR NES, Training_LR NES vs Acute_LR NES
...
pairs <- list(
  Training = list(
    ctr_x = "Training_HR", ctr_y = "Acute_HR",
    label_x = "Training", label_y = "Acute", title = "High Responders"
  ),
  Acute = list(
    ctr_x = "Training_LR", ctr_y = "Acute_LR",
...
SIG_COLORS <- c(
  "Sig Both"    = "#2E7D32",
  "Sig Training only" = "#4393C3",
  "Sig Acute only" = "#D6604D",
  "NS"          = "grey70"
)

SIG_LABEL_FILL <- c(
  "Sig Both"    = alpha("#2E7D32", 0.15),
  "Sig Training only" = alpha("#4393C3", 0.15),
  "Sig Acute only" = alpha("#D6604D", 0.15),
...
save_panel(pC, file.path(RPT, "panel_D_nes_scatter_MAIN"), PC_W, PC_H)
...
message("F06 Panel D (NES scatter) done")
