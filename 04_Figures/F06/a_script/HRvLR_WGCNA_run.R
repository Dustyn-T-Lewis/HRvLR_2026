...
pheno_cols <- intersect(
  c("X1RM_Leg_Pre", "mCSA_Pre", "fCSA_Mixed_Pre", "fCSA_Type_I_Pre",
...
write_baseline_cors <- function(stratum_label, stratum_subjects) {
  ...
  if (sum(sub_idx) < 5 || length(trait_cols) == 0) return(invisible(NULL))
  me_sub <- MEs[meta$sample_id[sub_idx], , drop = FALSE]
  trait_mat <- as.data.frame(dal_meta[match(meta$sample_id[sub_idx],
                                            dal_meta$Col_ID), trait_cols, drop = FALSE])
...
write_change_cors <- function(stratum_label, stratum_subjects) {
  ...
  if (length(common) < 3) return(invisible(NULL))
...
if (length(gt_levels) >= 4) {
  ...
  message(sprintf("LMM audit: %d tests (%d modules x %d contrasts)",
...
for (mc in setdiff(unique(module_colors), "grey")) {
  ...
  res <- tryCatch(
    run_ora_deduplicated(genes = mod_genes, universe = bg_genes,
  ...
  res$geneID      <- vapply(res$overlapGenes,
...
message(sprintf("Module enrichment: %d pathways across %d modules",
