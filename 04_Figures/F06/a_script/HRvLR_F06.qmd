...
- `module_phenotype_lmm.csv` — trajectory-moderation LMM per module (Panel S).
- `phenotype.csv` — the per-subject trait table.
- Panel PNG/PDF pairs in `b_reports/panels/`, the Panel S supplement in
  `b_reports/supp/`, the stitched `b_reports/F06_composite.png` / `.pdf`, and
  `c_data/F06_source_data.xlsx`.
...
## Supplement — trajectory moderation (LMM)

Panel C reduces each module to one T1→T3 delta before correlating. The
supplementary LMM keeps all three timepoints instead:
...
The `trait:timepoint` interaction is the direct test of trajectory moderation,
whether a module's whole time-course bends with the size of the response, which
the collapsed delta cannot ask. It carries the repeated-measures design through a
subject random intercept, matching the model the `05_test` GSVA sandbox uses on
pathway scores. The read-out is another null: no module-trait interaction clears
FDR. The `trait` main effect is reported alongside in `module_phenotype_lmm.csv`
but not surfaced in the panel, because a time-invariant trait can drive the
random-intercept variance to zero (the `singular` flag), which inflates that main
effect. The panel plots the interaction only, framed as the null it is.

![](../b_reports/supp/panel_s_phenotype_lmm.png)
