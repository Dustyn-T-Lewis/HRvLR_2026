...
fg <- read.xlsx(here("04_Figures", "F03_volcanoes", "c_data", "F03_volcanoes_source_data.xlsx"), sheet = "fgsea_all")
...
![F03. The five contrasts that test responder status: HR vs LR at baseline, trained and acute (A–C), and the two differential-response interactions (D–E).](../b_reports/F03_volcanoes.png)
...
- `b_reports/F03_volcanoes.{png,pdf}` — the five responder contrasts (three HRvLR + two interactions).
...
- `b_reports/supp/top30_<contrast>.{png,pdf}` — a no-dedup top-30 up / top-30 down figure
  per contrast.
- `c_data/F03_volcanoes_source_data.xlsx` — the fgsea cache plus the ring provenance.
...
in the pipeline: the cache is raw, so the rings and the F00_PILOT concordance read one
