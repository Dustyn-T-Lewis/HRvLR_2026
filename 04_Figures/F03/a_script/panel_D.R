...
write_csv(fgsea_export, file.path(DAT, "06_panel_D_fgsea_results.csv"))
...
PD_H <- max(100, n_facets * 40)

lbl_sz <- scale_text(BASE_COUNT, PD_W)
...
pD <- ggplot(count_df, aes(x = contrast, y = count, fill = direction))
...
write_csv(nes_summary, file.path(DAT, "audit_panel_D_nes_summary.csv"))

save_panel(pD, file.path(RPT, "panel_D_fgsea_MAIN"),
           width = PD_W, height = PD_H)
