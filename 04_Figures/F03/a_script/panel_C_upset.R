# Figure 3 — Panel C: UpSet Plot (Dual Bar + Dot Matrix) + Legend Key ----

if (!exists("meta")) source("04_Figures/F03/a_script/HRvLR_F03_setup.R")

# Panel C depends on Panel A for sig_sets, dir_map, all_genes, pi_total, fdr_total
...
write_csv(overlap_df, file.path(DAT, "audit_panel_C_overlap_enrichment.csv"))

pC_combined <- (pC_bars / pC_dots / pKeys) +
  plot_layout(heights = c(0.58, 0.22, 0.20))

save_panel(pC_combined, file.path(RPT, "panel_C_upset_MAIN"),
           width = PC_W, height = 160)
