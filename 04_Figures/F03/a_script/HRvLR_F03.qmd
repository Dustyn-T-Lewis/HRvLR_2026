...
![Between-group and interaction contrasts.](../b_reports/F03_differential.png)
...
- `b_reports/F03_differential.png` / `.pdf` — the five between-group and
...
## Why the split is 5 + 4

The nine contrasts are two different kinds of comparison, and mixing them
on one sheet would invite reading them as if they answered the same
question.

`F03_differential.png` holds the five that compare the two responder
groups or their responses: `Baseline_HRvLR`, `Trained_HRvLR`,
`Acute_HRvLR`, `Training_Interaction`, and `Acute_Interaction`. These are
where HR and LR differ, and where their exercise responses differ. This is
the responder story.
...
A between-group difference and a within-group change are not the same
measurement, so they get separate composites.
