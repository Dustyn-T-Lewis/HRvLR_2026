...
The Excel workbook is both the source-data export and the cache for the
downstream figures. F03 recomputes the fgsea enrichment fresh on every run,
seeded for reproducibility, and overwrites the workbook. The fgsea run is the slow
part of this figure, and the same enrichment feeds the F04 and F05 NES scatters,
which read the workbook instead of rerunning GSEA. So the three figures stay
consistent and the enrichment is paid for once per F03 run.
...
against the pathway collection fresh on every run, seeded for
reproducibility, then writes the result to the source-data workbook.
