...
F01_PANELS <- list()
F01_AUDIT <- list()
source(here("05_Figures", "F01_phenotype", "a_script", "setup.R"))
...
panel_dir <- here("05_Figures", "F01_phenotype", "a_script", "panels")
for (f in c("panel_a_volume", "panel_b_continuum", "panel_c_forest")) {
  source(file.path(panel_dir, paste0(f, ".R")))
}

source(here("05_Figures", "F01_phenotype", "a_script", "composite.R"))
