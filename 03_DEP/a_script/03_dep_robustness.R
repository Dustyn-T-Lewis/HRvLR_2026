...
run_response_comparison <- function(results_list, contrast_a, contrast_b, label) {
  ...
  diag <- tibble(
    comparison = label,
    test = c(
      "Kolmogorov-Smirnov", "Fligner-Killeen",
      "Wilcoxon signed-rank", "Cliff's delta"
    ),
    statistic = c(
      ks_res$statistic, fk_res$statistic,
    ...
    interpretation = c(
      ...
      sprintf(
        "%s effect (delta = %.3f; >0 = %s responds more)",
...
rp_training <- run_response_comparison(
  results_list,
  "Training_HR", "Training_LR", "Training (HR vs LR)"
)
rp_acute <- run_response_comparison(
  results_list,
...
power_df <- map_dfr(contrast_names, function(cname) {
  ...
  eff_sigma <- if (paired && !is.na(within_cor)) {
    sigma_residual * sqrt(2 * (1 - within_cor))
  } else {
...
if (file.exists(cfg$imp_rds)) {
  ann <- as.data.frame(dal$annotation)
  mat <- dal$data

  imp_dal <- readRDS(cfg$imp_rds)
  imp_mat <- as.matrix(imp_dal$data) # rownames = uniprot_id
  ...
  for (cname in contrast_names) {
    ...
    merged <- inner_join(
      comb |> select(uniprot_id,
        t_nonimp = all_of(t_col),
...
hdr_style <- createStyle(
  textDecoration = "italic", fontColour = "#555555",
  wrapText = TRUE
)
...
write_supp_sheet <- function(wb, sheet, header, data) {
  ...
  addStyle(wb, sheet, hdr_style,
    rows = seq_along(header), cols = 1,
    stack = TRUE
  )
...
for (cname in contrast_names) {
  ...
  write_supp_sheet(wb_supp, cname, c(
    ...
    sprintf(
      "Proteins tested: %d | FDR<0.10: %d | Pi<0.05: %d",
