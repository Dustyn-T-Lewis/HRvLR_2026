# panel_I.R — Acute Scatter: Acute_HR vs Acute_LR logFC
# Protein-level concordance of acute exercise response between responder groups.
...
here::i_am(".here")
source(here::here("04_Figures/F04/a_script/style_f2.R"))
...
scatter_df <- dep_df |>
  transmute(
    gene,
    logFC_HR       = logFC_Acute_HR,
    logFC_LR       = logFC_Acute_LR,
    pi_HR          = pi_score_Acute_HR,
    pi_LR          = pi_score_Acute_LR,
    pi_Interaction = pi_score_Acute_Interaction
  ) |>
...
write_csv(stats_df, file.path(DAT, "panel_I", "acute_scatter_stats.csv"))
...
# ── Plot ──────────────────────────────────────────────────────────────────────
pI <- ggplot(mapping = aes(x = logFC_HR, y = logFC_LR)) +
  ...
  labs(
    title    = "Acute Response: HR vs LR — Protein-Level Concordance",
    subtitle = sprintf(
    ...
    x = expression(log[2]*FC ~ "(Acute HR: T3 \u2212 T1)"),
    y = expression(log[2]*FC ~ "(Acute LR: T3 \u2212 T1)")
  ) +
  FIG_THEME +
  theme(legend.position = "none")

save_panel(pI, file.path(RPT, "panel_I_acute_scatter_MAIN"), PW, PW)

scatter_df |>
  transmute(gene,
            logFC_Acute_HR       = round(logFC_HR, 4),
            logFC_Acute_LR       = round(logFC_LR, 4),
            pi_HR                = round(pi_HR, 6),
            pi_LR                = round(pi_LR, 6),
            pi_Interaction       = round(pi_Interaction, 6),
            significance         = as.character(significance),
            quadrant) |>
  arrange(significance, desc(abs(logFC_Acute_HR) + abs(logFC_Acute_LR))) |>
  write_csv(file.path(DAT, "panel_I", "acute_scatter.csv"))

message("Panel I done: Acute scatter")
